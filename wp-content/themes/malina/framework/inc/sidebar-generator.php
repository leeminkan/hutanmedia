...
class sidebar_generator {
	...
	public function __construct() {
		...
		//edit posts/pages
		//add_action('edit_form_advanced', array('sidebar_generator', 'edit_form'), 110);
		//add_action('edit_page_form', array('sidebar_generator', 'edit_form'), 110);
			...
			<script>
				function add_sidebar( sidebar_name )
				{
					...
					var mysack = new sack("<?php echo site_url(); ?>/wp-admin/admin-ajax.php" );    
				  	...
				  	mysack.onError = function() { alert("<?php esc_html_e('Ajax error. Cannot add sidebar','malina'); ?> ")};
				  	mysack.runAJAX();
					return true;
				}
	...
	public static function add_sidebar(){
		...
		if(isset($sidebars[$id])){
			die("alert('". esc_html__('Sidebar already exists, please use a different name.','malina')."')");
		}
		...
		$js = "
			...
			//var textNode = document.createTextNode('[<a href=\'javascript:void(0);\' onclick=\'return remove_sidebar_link($name);\'>". esc_html__('Remove','malina')."</a>]');
			//cellLeft.appendChild(textNode)
	...
	public static function remove_sidebar(){
		...
		$id = sidebar_generator::name_to_class($name);
		if(!isset($sidebars[$id])){
			die("alert('". esc_html__('Sidebar does not exist.','malina')."')");
		}
	...
	public static function admin_page(){
		...
		<script>
			function remove_sidebar_link(name,num){
				answer = confirm("<?php esc_html_e('Are you sure you want to remove','malina'); ?> " + name + "<?php esc_html_e('?\nThis will remove any widgets you have assigned to this sidebar.','malina'); ?>");
				if(answer){
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt("<?php esc_html_e('Sidebar Name:','malina'); ?>","");
				//alert(sidebar_name);
				add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2><?php esc_html_e('Sidebar Generator','malina'); ?></h2>
			
			<p><?php esc_html_e('You can generate as many sidebars as you need. Then you can place them on any page you wish.','malina'); ?></p>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php esc_html_e('Name','malina'); ?></th>
					<th><?php esc_html_e('CSS class','malina'); ?></th>
					<th><?php esc_html_e('Remove','malina'); ?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<tr class="<?php echo ''.$alt?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo ''.$sidebar; ?>',<?php echo ''.($cnt+1); ?>);" title="<?php esc_attr_e('Remove this sidebar','malina'); ?>"><?php esc_html_e('remove','malina'); ?></a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php esc_html_e('No Sidebars defined','malina'); ?></td>
					</tr>
					<?php
				}
				?>
			...
			<div class="add_sidebar">
				<a href="javascript:void(0);" class="button" onclick="return add_sidebar_link()" title="<?php esc_attr_e('Add a sidebar','malina'); ?>"><?php esc_html_e('Create Sidebar','malina'); ?></a>
			</div>
		</div>
		<?php
	}
	...
	*/
	public static function save_form($post_id){
		...
		if(!empty($is_saving)){
			$page_sidebar = rwmb_get_value('malina_page_sidebar');
			delete_post_meta($post_id, 'sbg_selected_sidebar');
			delete_post_meta($post_id, 'sbg_selected_sidebar_replacement');
			add_post_meta($post_id, 'sbg_selected_sidebar', $page_sidebar);
			add_post_meta($post_id, 'sbg_selected_sidebar_replacement', $page_sidebar);
		}		
	}
	...
	<div id='sbg-sortables' class='meta-box-sortables'>
		<div id="sbg_box" class="postbox " >
			<div class="handlediv" title="<?php esc_attr_e('Click to toggle','malina'); ?>"><br /></div><h3 class='hndle'><span><?php esc_html_e('Sidebars','malina'); ?></span></h3>
			<div class="inside">
				<div class="sbg_container">
					<input name="sbg_edit" type="hidden" value="sbg_edit" />
					
					<p>
						<?php esc_html_e("Select the sidebar you wish to display on this page. If you don't want to display a Sidebar, select the 'Full-Width' Template.", "malina"); ?>
						<br />
						<strong><?php esc_html_e('Note', 'malina'); ?>:</strong><?php esc_html_e("You must first create the sidebar under Appearance > Sidebars.",'malina'); ?>
					</p>
					<ul>
					<?php 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator[<?php echo ''.$i?>]" style="display: none;">
								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php esc_html_e('WP Default Sidebar','malina'); ?></option>
							<?php
							...
							unset($sidebar_replacements['wpmm']);
