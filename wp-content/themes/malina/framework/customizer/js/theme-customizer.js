(function( $ ) {
	...
	api( 'malina_header_border_color', function( value ) {
	    value.bind( function( to ) {
	        $( ' #header, .header-top, #header.header8 .container:not(.header-fullwidth) #navigation-block, #header.header8 .container.header-fullwidth' ).css('border-color', to);
	    });
	});
	api( 'malina_header_top_border_color', function( value ) {
	    value.bind( function( to ) {
	        $( '#header-main' ).css('border-color', to);
	    });
	});
	...
	api( 'malina_menu_letter_spacing', function( value ) {
	    value.bind( function( to ) {
	        $( '.menu li a, .wp-megamenu li a' ).css('letter-spacing', to+'px');
	    });
	});
	...
	api( 'malina_footer_copyright_font_size', function( value ) {
	    value.bind( function( to ) {
	        $( '#footer-copy-block' ).css('font-size', to);
	    });
	});
	api( 'malina_footer_copyright_font_family', function( value ) {
	    value.bind( function( to ) {
	    	var tmp = to.replace(/\s/g, '+');
	    	if( !$('link#google-font').length ){
	    		$('head').append('<link rel="stylesheet" id="google-font" href="#" type="text/css" media="all">');
	    	...
	    	$( '#footer-copy-block' ).css('font-family', to);
	    });
	});
	api( 'malina_footer_copyright_color', function( value ) {
	    value.bind( function( to ) {
	        $( '#footer-copy-block' ).css('color', to);
	    });
	});
	api( 'malina_posts_headings_font_size', function( value ) {
	    value.bind( function( to ) {
	    	to = parseInt(to);
	    	$( '.title h1, .title h2, .title h3' ).css('font-size', to+'px');
	    });
	});
	api( 'malina_posts_headings_font_weight', function( value ) {
	    value.bind( function( to ) {
	        $( '.title h1, .title h2, .title h3' ).css('font-weight', to);
	    });
	});
	api( 'malina_posts_headings_font_family', function( value ) {
	    value.bind( function( to ) {
	    	...
	    	$( '.title h1, .title h2, .title h3' ).css('font-family', to);
	    });
	});
	api( 'malina_posts_headings_item_color', function( value ) {
	    value.bind( function( to ) {
	        $( '.title h1 a, .title h1, .title h2, .title h2 a, .title h3 a' ).css('color', to);
	    });
	});
	api( 'malina_posts_headings_item_color_active', function( value ) {
	    value.bind( function( to ) {
	        $( '.title h1 a, .title h2 a, .title h3 a, .related-item-title a' ).hover(function(){$(this).css('color', to)}, function(){$(this).css('color', '')});
	    });
	});
	...
	api( 'malina_grid_posts_font_size', function( value ) {
	    value.bind( function( to ) {
	        $( '.blog-posts .post .post-content' ).css('font-size', to);
	    });
	});
	api( 'malina_grid_posts_line_height', function( value ) {
	    value.bind( function( to ) {
	        $( '.blog-posts .post .post-content' ).css('line-height', to);
	    });
	});
	api( 'malina_grid_posts_font_family', function( value ) {
	    value.bind( function( to ) {
	    	var tmp = to.replace(/\s/g, '+');
	    	if( !$('link#google-font-12').length ){
	    		$('head').append('<link rel="stylesheet" id="google-font-12" href="#" type="text/css" media="all">');
	    	}
	    	if(to != 'Arial' && to != 'Verdana' && to != 'Trebuchet' && to != 'Georgia, sans-serif' && to != 'Times New Roman, sans-serif' && to != 'Tahoma' && to != 'Palatino' && to != 'Helvetica, sans-serif'){
	    		$('link#google-font-12').attr({href:"https://fonts.googleapis.com/css?family="+tmp })
	    	}
	    	$( '.blog-posts .post .post-content' ).css('font-family', to);
	    });
	});
	api( 'malina_body_color', function( value ) {
	    value.bind( function( to ) {
	        $( '.blog-posts .post .post-content' ).css('color', to);
	    });
	});//----------
	...
	api( 'malina_body_background_color', function( value ) {
	    value.bind( function( to ) {
	        $( 'body[class*="header-version"]' ).css('background-color', to);
	    });
	});
	api( 'malina_links_color', function( value ) {
	    value.bind( function( to ) {
	        $( '.post-content a' ).css('color', to);
	    });
	});
	api( 'malina_links_color_hover', function( value ) {
	    value.bind( function( to ) {
	        $( '.post-content a' ).hover(function(){$(this).css('color', to)}, function(){$(this).css('color', '')});
	    });
	});

	api( 'malina_page_title_font_size', function( value ) {
	    value.bind( function( to ) {
	    	to = parseInt(to);
	        $( '.page-title h2' ).css('font-size', to+'px');
	    });
	});
	api( 'malina_page_title_font_weight', function( value ) {
	    value.bind( function( to ) {
	        $( '.page-title h2' ).css('font-weight', to);
	    });
	});
	api( 'malina_page_title_font_family', function( value ) {
	    value.bind( function( to ) {
	    	var tmp = to.replace(/\s/g, '+');
	    	if( !$('link#google-font-page-title').length ){
	    		$('head').append('<link rel="stylesheet" id="google-font-page-title" href="#" type="text/css" media="all">');
	    	}
	    	if(to != 'Arial' && to != 'Verdana' && to != 'Trebuchet' && to != 'Georgia, sans-serif' && to != 'Times New Roman, sans-serif' && to != 'Tahoma' && to != 'Palatino' && to != 'Helvetica, sans-serif'){
	    		$('link#google-font-page-title').attr({href:"https://fonts.googleapis.com/css?family="+tmp+":100,100italic,200,200italic,300,300italic,400,400italic,600,600italic,700,700italic,800,800italic&amp;subset=latin,latin-ext,cyrillic,cyrillic-ext,greek-ext,greek,vietnamese"})
	    	}
	    	$( '.page-title h2' ).css('font-family', to);
	    });
	});
	api( 'malina_page_title_item_color', function( value ) {
	    value.bind( function( to ) {
	        $( '.page-title h2' ).css('color', to);
	    });
	});
	api( 'malina_page_title_letter_spacing', function( value ) {
	    value.bind( function( to ) {
	    	to = parseInt(to);
	        $( '.page-title h2' ).css('letter-spacing', to+'px');
	    });
	});
	api( 'malina_page_title_transform', function( value ) {
	    value.bind( function( to ) {
	    	to = parseInt(to);
	        $( '.page-title h2' ).css('text-transform', to);
	    });
	});
