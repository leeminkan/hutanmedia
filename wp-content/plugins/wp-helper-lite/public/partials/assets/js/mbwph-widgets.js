(function ($) {
    if (!window.MBwph) window.MBwph = {};

    MBwph.CloneWidgets = {
        // Initialize
        init: function () {
            $('body').on('click', '.widget-control-actions .clone-me', MBwph.CloneWidgets.Clone);
            MBwph.CloneWidgets.Bind();
        },

        // Add Clone button to widgets control buttons
        Bind: function () {
            $('#widgets-right').off('DOMSubtreeModified', MBwph.CloneWidgets.Bind);
            $('.widget-control-actions:not(.mbwph-cloneable)').each(function () {
                var $widget = $(this);

                var $clone = $('<a>');
                var clone = $clone.get()[0];
                $clone.addClass('clone-me mbwph-clone-action')
                    .attr('title', mbwph_duplicate_widgets.title)
                    .attr('href', '#')
                    .html(mbwph_duplicate_widgets.text);
                $widget.addClass('mbwph-cloneable');
            ...
            $('#widgets-right').on('DOMSubtreeModified', MBwph.CloneWidgets.Bind);
        },
        ...
        Clone: function (ev) {
            ...
            if ($widget.find('.text-widget-fields').length > 0) {
                ...
                wp.editor.initialize('e_' + timeStamp + '_text', {tinymce: true, quicktMBwphs: true});
            ...
            ev.stopPropMBwphation();
            ev.preventDefault();
        }
    };
    $(MBwph.CloneWidgets.init);
})(jQuery);