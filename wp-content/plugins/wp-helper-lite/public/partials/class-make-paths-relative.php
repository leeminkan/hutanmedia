 ...
 */
if ( defined( 'ABSPATH' ) && ! class_exists( 'mbwph_Make_Paths_Relative_Frontend' ) ) {	
	class mbwph_Make_Paths_Relative_Frontend {
		...
		*/
		...
		$this->make_paths_relative_applied();
		}
		...
		* @return void
		*/
		private function make_paths_relative_applied(  ) {

		//Filters to make the permalinks to relative
		add_filter( 'the_permalink',array( $this, 'make_paths_relative_remove' ), 100);
		add_filter( 'post_link',array( $this, 'relative_post_urls' ), 100, 3);
		add_filter( 'post_type_link',array( $this, 'relative_post_urls' ), 100, 3);
		add_filter( 'wpseo_xml_sitemap_post_url',array( $this, 'sitemap_post_url' ), 100);   

		add_filter( 'page_link',array( $this, 'make_paths_relative_remove' ), 100);
		add_filter( 'page_type_link',array( $this, 'make_paths_relative_remove' ), 100, 2);
		add_filter( 'get_archives_link', array( $this, 'make_paths_relative_remove' ), 100 );
		add_filter( 'author_link',array( $this, 'make_paths_relative_remove' ), 100 );
		add_filter( 'term_link',array( $this, 'make_paths_relative_remove' ), 100, 3 );

		//Filters to make the scripts and style urls to relative    
		add_filter( 'script_loader_src',array( $this, 'make_paths_relative_remove' ), 100);
		add_filter( 'style_loader_src',array( $this, 'make_paths_relative_remove' ), 100);    

		//Filter to make the media(image) src to relative
		add_filter( 'wp_get_attachment_url',array( $this, 'make_paths_relative_remove' ), 100);
		add_filter( 'wp_calculate_image_srcset',array( $this, 'make_paths_relative_remove_srcset' ), 100);
		}
